...
class VisibilityConverterTest(utils.TestBase):
    ...
    def setUp(self):
        ...
        # default visparam
        self.visparam = core.VisParamContainer(vis=self.vis)
    ...
    def test_channel_interp(self):
        ...
        # (u,v) = (0,0) maps onto grid plane center
        self.assertTrue(ws.u[0] == (self.imageparam.imsize[0] - 1) / 2.0)
        self.assertTrue(ws.v[0] == (self.imageparam.imsize[1] - 1) / 2.0)
    ...
    def test_parallel(self):
        # create reader
        reader = core.VisibilityReader(self.visparam)
    ...
    def get_converter(self, start, width, nchan):
        self.imageparam = core.ImageParamContainer(imagename=self.imagename,
                                              start=start,
                                              width=width, 
                                              nchan=nchan,
        ...
        converter = core.VisibilityConverter(self.visparam, self.imageparam)
        return converter
...
#     
    ...
    def _test_lsr_freq(self, mfr='TOPO'):
        ...
        with casa.OpenTableForReadWrite(os.path.join(self.vis, 'SPECTRAL_WINDOW')) as tb:
            ...
            self.assertTrue(mfr in core.VisibilityConverter.frequency_reference)
            mfr_id = core.VisibilityConverter.frequency_reference.index(mfr)
            if not numpy.all(mfrs == mfr_id):
                mfrs[:] = mfr_id
                tb.putcol('MEAS_FREQ_REF', mfrs)

        visparam = core.VisParamContainer(vis=self.vis)
        imageparam = core.ImageParamContainer(imagename=self.imagename,
                                         phasecenter=str(self.field_id))
        converter = core.VisibilityConverter(visparam, imageparam)
