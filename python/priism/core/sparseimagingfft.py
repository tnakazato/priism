...
class SparseImagingExecutor(object):
    ...
    def _show_result(self, mfista_result):
        ...
        logger.info(' size of input vector:  {0}'.format(mfista_result.M))
        logger.info(' size of output vector: {0}'.format(mfista_result.N))
        if mfista_result.NX != 0:
            logger.info('size of image:          {0} x {1}'.format(mfista_result.NX,
                                                                    mfista_result.NY))
        logger.info('\n\n Problem Setting:\n\n')
